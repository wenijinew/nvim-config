...
M.get_filename = function()
    ...
    if not f.isempty(filename) then
        ...
        if f.isempty(file_icon) then
            log.debug('not file icon for ' .. filename .. ', to use ' .. icons.kind.File)
...
M.get_winbar = function(append_mode)
    if excludes() then
        return
    end
    local f = require "utils.functions"
    ...
    if not f.isempty(value) and append_mode then
        local mod = "%#NavicSeparator#" .. icons.ui.Target .. "%*"
        if gps_added then
            value = value .. " " .. mod
        else
            value = value .. mod
...
M.set_winbar = function(append_mode)
    local value = M.get_winbar(append_mode)
...
M.create_winbar = function()
    ...
    vim.api.nvim_create_augroup("_resetwinbar", {})
        ...
        vim.api.nvim_create_autocmd(
            { "CursorHoldI", "CursorHold", "BufWinEnter", "BufFilePost", "InsertEnter", "BufWritePost", "TabClosed" },
            {
                ...
                callback = function()
                    ...
                    if not status_ok then
                        require("setup.breadcrumbs").set_winbar(true)
                    end
                end,
            }
        )
        vim.api.nvim_create_autocmd(
            { "BufLeave" },
            {
                ...
                callback = function()
                    local status_ok, _ = pcall(vim.api.nvim_buf_get_var, 0, "lsp_floating_window")
                    if not status_ok then
                        require("setup.breadcrumbs").set_winbar(false)
                    end
                end,
            }
        )
